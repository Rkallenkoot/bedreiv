...
<div class="container-fluid">
	<div class="row">
		...
		<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<form class="form-horizontal col-sm-8" action="/configs/hardware/create" method="POST">
				<legend>Hardware toevoegen</legend>
				<div class="form-group">
					<label for="id" class="col-sm-2 control-label">Hardware ID:</label>
					<div class="col-sm-10">
						<input type="text" class="form-control" name="id" id="id" placeholder="Hardware ID">
					</div>
				</div>
				<div class="form-group">
					<label for="jaarvanaanschaf" class="col-sm-2 control-label">Jaar van aanschaf:</label>
					<div class="col-sm-10">
						<input type="number" class="form-control" name="jaarvanaanschaf" id="jaarvanaanschaf" value="<?php echo date("Y");?>">
					</div>
				</div>
					...
					<div class="col-sm-10">
						<select class="form-control" name="locatie">
							<?php foreach ($data['locaties'] as $locatie):?>
								<option value="<?=$locatie['id']?>"><?=$locatie['lokaal']?></option>
							<?php endforeach; ?>
						</select>
					</div>
				</div>
					...
					<div class="col-sm-10">
						<select class="form-control" name="merk">
							<?php foreach ($data['merken'] as $merk):?>
								<option value="<?=$merk['id']?>"><?=$merk['naam']?></option>
							<?php endforeach; ?>
						</select>
					</div>
				</div>
